...
async function loadDashboard() {
  ...
  el.innerHTML = loans.map(p => {
    ...
    return `<div class="loan-card ${venc?'overdue':''}">
      <div><div class="loan-title">${p.documento}</div><div class="loan-meta">Solicitante: <strong>${p.solicitante}</strong> · ${p.archivo} · Dev. esperada: ${p.fecha_devolucion}</div></div>
